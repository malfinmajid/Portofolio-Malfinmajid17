    ...
    return (
       <nav
            ...
            <div className="max-w-330 mx-auto px-5">
                <div className="flex items-center justify-between">
                    {/* Logo */}
                        ...
                        {/* Dekstop Navigation*/}
                        <nav className='hidden md:flex items-center gap-7 mr-15'>
                            {Nav_Links.map((link) => (
                                <button
                                    key={link.id}
            ...
            >
                ...
                <div  className="bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3">{Nav_Links.map((link) => (
                        <button key={link.id}
                            onClick={() => handleNavClick(link.id)}
                    ...
                    <button 
                            ...
                            className='w-full px-7 py-3.5 bg-primary/10 text-white font-medium text-base rounded-[17px] border border-primary hover:bg-primary/50 transition-all duration-300 mt-2'>Hire Me
                        </button>
                </div>
            </div>
        </nav>
